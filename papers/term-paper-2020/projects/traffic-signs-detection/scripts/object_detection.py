...
def draw_boxes(image, boxes, obj_thresh):
    for box in boxes:
        label = -1
...
def process_image(model, image):
    ...
    print("Image preprocessing time: " + str(time.time() - start))
    ...
    for i in range(len(prediction)):
        boxes += decode_netout(prediction[i][0], anchors[i], obj_thresh, net_h, net_w)
    print("Decoding time: " + str(time.time() - start))

    start = time.time()
    # correct the sizes of the bounding boxes
    correct_yolo_boxes(boxes, image_h, image_w, net_h, net_w)
    print("Correcting time: " + str(time.time() - start))

    start = time.time()
    # suppress non-maximal boxes
    do_nms(boxes, nms_thresh)
    print("Suppressing time: " + str(time.time() - start))

    start = time.time()
    final = draw_boxes(image, boxes, obj_thresh)
    print("Final drawing time: " + str(time.time() - start))
